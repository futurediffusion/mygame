...
## Novedades desde R3
- **Combo de salto perfecto:** `JumpModule` abre una ventana breve tras aterrizar y, si vuelves a saltar en el momento justo, aumenta progresivamente la altura del salto y la velocidad horizontal hasta un máximo configurable de 100 cadenas, reiniciándose ante un salto normal. `MovementModule` aplica el multiplicador antes de mover al cuerpo para conservar la sincronía con el SimClock.【F:Modules/Jump.gd†L6-L178】【F:Modules/Movement.gd†L1-L113】
...
- `MovementModule`: aceleración suelo/aire, fast fall y ajuste progresivo de velocidad según pendiente.【F:Modules/Movement.gd†L1-L113】
- `JumpModule`: buffer + coyote, salto variable, animación/audio y restauración de `floor_snap` tras despegar.【F:Modules/Jump.gd†L1-L183】
...
### Sistema de combate (ataques/daño/vida)
- `AttackModule` implementa un combo de tres golpes (`P1`→`P2`→`P3`) con datos de paso (`STEP_DATA`) que definen duración, ventana de golpe, bloqueo de movimiento, daño e impulso aplicados sobre el objetivo. Controla buffering, cooldown entre cadenas, bloqueo de movimiento y evita múltiplos golpes usando `already_hit`, además de exponer señales (`attack_started`, `attack_window_opened`, `attack_hit`, `attack_finished`) para reaccionar desde otros nodos.【F:Modules/AttackModule.gd†L4-L540】【F:Modules/AttackModule.gd†L745-L804】
- `AttackCallBridge` enlaza `AnimationTree`/`AnimationPlayer` con el módulo: monitoriza parámetros activos, abre/cierra la ventana de impacto calculando offsets según `speed_scale` y delega en `AttackModule.attack_start_hit()`/`attack_end_hit()`. Si el proyecto usa keyframes en el `AnimationPlayer`, estos métodos también pueden llamarse desde pistas del timeline.【F:scripts/combat/AttackCallBridge.gd†L1-L210】
- `AttackHitbox` se asigna a manos/antebrazos mediante `ATTACK_TO_HITBOX`: activa detección únicamente cuando `AttackModule.hit_active` es verdadero, filtra al propietario para evitar auto-daño y reenvía `on_attack_overlap` con normal del golpe. El área trabaja sobre las capas `PLAYER`, `ALLY` y `ENEMY`, por lo que cualquier entidad objetivo necesita `CharacterBody3D` y formar parte de esas máscaras.【F:scripts/combat/AttackHitbox.gd†L1-L150】【F:Modules/AttackModule.gd†L730-L804】
- `_apply_damage_to_target()` busca un nodo `Health` hijo del cuerpo golpeado y, si lo encuentra, invoca `take_damage()` con la fuente; en su defecto recurre al método `apply_damage()` del cuerpo. La vida del jugador y enemigos se implementa con el recurso `Health.gd`, que soporta invulnerabilidad temporal, señales `damaged`/`died` y sincroniza el HUD mediante `health_changed` en `art/ui/HUD.gd` y `player.gd`/`Enemy.gd` como fallback para escenas sin nodo único.【F:Modules/AttackModule.gd†L785-L804】【F:scripts/combat/Health.gd†L1-L52】【F:scenes/entities/player.gd†L198-L228】【F:art/ui/HUD.gd†L55-L118】
- `EnemyBrain` gobierna ataques cuerpo a cuerpo midiendo distancias planas; cuando está en rango llama a `trigger_attack_request()` en el módulo (con `accepts_input = false`) y reinicia un cooldown `reattack_cooldown`. Esto reutiliza exactamente el mismo pipeline del jugador y permite que las animaciones compartidas disparen golpes vía `AttackCallBridge`.【F:scripts/enemy/EnemyBrain.gd†L1-L200】【F:scripts/enemy/Enemy.gd†L1-L120】
...
res://
 ...
 ├─ tests/ (escenas headless para SimClock, FastFall, DataIsolation)
```
...
- Tests críticos corren en headless (`TestFastFall`, `TestDataIsolation`) y dependen del orden de tick del `SimClock`. Manténlos actualizados tras tocar prioridades, física o manejo de datos.【F:tests/TestFastFall.tscn†L1-L8】【F:tests/TestDataIsolation.gd.uid†L1-L1】
