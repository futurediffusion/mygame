...
- Sneak ahora se dirige con `scenes/entities/SneakAnimController.gd` (tecla `C`): el Player delega el toggle a este nodo hijo que mezcla `SneakBlend`, interpola `EnterHandoff` vía tween y sincroniza `SneakIdleWalk`/`SneakIdleWalk2` con la velocidad actual. `Modules/AnimationCtrl.gd` detecta el controlador y deja de escribir los blends internos, así que cualquier ajuste debe ir a través del script dedicado (usa tabs y los paths `parameters/...`; ya contempla el prefijo `LocomotionSpeed` como fallback).
- Si la acción `toggle_sneak` no estuviera presente (InputMap legado), deja el fallback en `Player._cache_input_states()` para usar `crouch.just_pressed`; evita retirar ese respaldo porque mantiene funcional la tecla `C` aun cuando falte el bootstrap.
...
- Si necesitas tweenear parámetros del `AnimationTree`, pasa rutas como `NodePath`; convierte los `StringName` con `NodePath(String(path))` antes de invocar `tween_property`, de lo contrario Godot 4.4 marca error de argumento inválido.
