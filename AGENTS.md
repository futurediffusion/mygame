...
- Sneak ahora es un contexto toggleado (tecla `C`): `player.gd` guarda `crouch_record["active"]`, expone `is_sneaking()` y emite `context_state_changed`; `Modules/AnimationCtrl.gd` escucha la señal, viaja a `Sneak`→`SneakExit`, dispara el OneShot `SneakEnter` y bloquea el sprint mientras `_sneak_active` sea verdadero.
