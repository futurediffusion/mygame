...
- Sneak ahora es un contexto toggleado (tecla `C`): `player.gd` guarda `crouch_record["active"]`, expone `is_sneaking()` y emite `context_state_changed`; `Modules/AnimationCtrl.gd` viaja entre `LocomotionSpeed`↔`Sneak`, dispara el OneShot `SneakEnter`, mezcla `SneakBlend.blend` para cubrir la entrada y controla `LocomotionSpeed/ExitBlend/blend` mientras se reproduce `sneak_exit` (sin depender de un estado `SneakExit` separado) antes de volver a locomoción.
- Mezclar tabs con espacios en `Modules/AnimationCtrl.gd` (especialmente dentro de `_update_exit_blend()`) provoca el error de parseo "Unexpected ""else"" in class body" en Godot 4.4. Verifica que cada nivel anidado use tabs al editar el módulo para conservar `class_name AnimationCtrlModule`.
- En `_update_exit_blend()` evita depender de `else` anidados tras un `return`: Godot 4.4 llegó a interpretar el `else` como si estuviera al nivel de clase. Prefiere una rama explícita (`if _exit_blend_target < 1.0:`) con tabs limpios para que el módulo siga registrando su `class_name`.
