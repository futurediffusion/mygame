...
func _ready() -> void:
	...
	state.setup(player)
	var initial_y: float = -2.0
	player.velocity = Vector3(0.0, initial_y, 0.0)
	var dt: float = 0.1
	state.pre_move_update(dt)
	var expected_y: float = initial_y - player.gravity * state.fall_gravity_scale * dt
	...
	state.pre_move_update(dt)
	var expected_cross: float = 0.5 - player.gravity * dt
	if expected_cross < 0.0:
		var fall_scale: float = max(state.fall_gravity_scale, 1.0)
	...
	var input_dir: Vector3 = Vector3(1.0, 0.0, 0.0)
	...
	var falling_speed: float = Vector2(player.velocity.x, player.velocity.z).length()
	var expected_fall_speed: float = movement.max_speed_air * movement.fast_fall_speed_multiplier
	...
	player.velocity = Vector3.ZERO
	var ground_iterations: int = 120
	for _j in range(ground_iterations):
		player.velocity.y = 0.0
		movement.set_frame_input(input_dir, false)
		movement.physics_tick(sim_dt)
	var ground_speed: float = Vector2(player.velocity.x, player.velocity.z).length()
	var expected_ground: float = movement.max_speed_ground
